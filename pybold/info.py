...
def _import_module_with_version_check(
    ...
    try:
        ...
        user_friendly_info = ('Module "{0}" could not be found. {1}').format(
            module_name,
            install_info or 'Please install it properly to use pyBOLD.')
    ...
    if version_too_old:
        message = (
            ...
            'is required to use pyBOLD. {module_version} was found. '
...
def _check_module_dependencies(is_pyta_installing=False):
    """Throw an exception if pyBOLD dependencies are not installed.
